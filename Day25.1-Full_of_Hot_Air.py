...
#with open('Day25-Input--Debug', 'r') as file:
with open('Day25-Input', 'r') as file:
    for line in file:
        line = line.rstrip()
        no_digits = len(line)
        print(line)
        for base in range(no_digits-1, -1, -1):
            digit = int(line[(base+1)*-1].replace('-', '-1').replace('=', '-2'))
            fuelsum += digit * 5 ** base
        
print(fuelsum)
for base in range(20, 0, -1):
    for val in range(2, -3, -1):
        if fuelsum - val * 5 ** base >= -3 * 5 ** (base-1) + 1:
            fuelsum -= val * 5 ** base
            out += str(val).replace('-1', '-').replace('-2', '=')
            break
out = out.lstrip('0')
out += str(fuelsum).replace('-1', '-').replace('-2', '=')
print()
print(out)

# Examples are fine. 2=-1=0
