...
with open('Day11-Input', 'r') as file:
    while True:
        ...
        items = []
        file.readline()
        for i in file.readline().split()[2:]:
            items.append(int(i.strip(',')))
        md["items"] = items
        line = file.readline()[23:]
        md["op"] = line[0]
        md["param"] = line[2:-1]
        d = int(file.readline()[21:])
        alldivs *= d
        md["div"] = d
        md["true"] = int(file.readline()[29:])
        md["false"] = int(file.readline()[30:])
        md["count"] = 0
        monkeys.append(md)
    ...
    for turn in range(len(monkeys)):
        ...
        for item in md["items"]:
            ...
            if worry % md["div"] == 0:
                ...
                monkeys[md["false"]]["items"].append(worry)
        monkeys[turn]["items"] = []
